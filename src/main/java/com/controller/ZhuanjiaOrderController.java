...
import com.proxy.OrderAuditProxy;
import com.proxy.OrderAuditProxyImpl;
...
public class ZhuanjiaOrderController {
    ...
    /***************** 新增（开始） *****************/
    private final OrderAuditProxy auditProxy;

    public ZhuanjiaOrderController() {
        this.auditProxy = new OrderAuditProxyImpl(this);
    }
    /***************** 新增（结束） *****************/
    ...
    public R doShenhe(ZhuanjiaOrderEntity order, HttpServletRequest request) {
        try {
            if (order.getZhuanjiaOrderYesnoTypes() == 3) {
                HuiyuanEntity huiyuanEntity = huiyuanService.selectById(order.getHuiyuanId());
                ZhuanjiaEntity zhuanjiaEntity = zhuanjiaService.selectById(order.getZhuanjiaId());
                if (huiyuanEntity != null && zhuanjiaEntity != null) {
                    huiyuanEntity.setNewMoney(huiyuanEntity.getNewMoney() + zhuanjiaEntity.getGuahaoMoney());
                    huiyuanService.updateById(huiyuanEntity);
                }
            }
            zhuanjiaOrderService.updateById(order); // 根据id更新
            return R.ok();
        } catch (Exception e) {
            logger.error("审核处理失败: {}", e.getMessage());
            return R.error(500, "审核处理失败: " + e.getMessage());
        }
    }
